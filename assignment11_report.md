...
Điểm chính trong thiết kế này là đa số tấn công nên bị chặn ngay ở input layer. Đây là lựa chọn tốt vì nó rẻ hơn, nhanh hơn và an toàn hơn so với để mô hình sinh câu trả lời rồi mới xử lý ở output. Tuy nhiên, output guardrails và judge vẫn rất quan trọng vì chúng đóng vai trò lớp bảo vệ thứ hai nếu input layer bỏ sót.

## 2. Phân tích false positive

Trong pipeline hiện tại, các truy vấn ngân hàng an toàn được kỳ vọng sẽ đi qua bình thường. Ở lần chạy thử, hai truy vấn an toàn đầu tiên đã pass và nhận điểm judge rất cao ở cả bốn tiêu chí: safety, relevance, accuracy, và tone. Điều đó cho thấy chính sách cơ bản vẫn sử dụng được cho các câu hỏi ngân hàng thông thường.
...
Cách tiếp cận nhiều lớp như vậy thực tế hơn, linh hoạt hơn, và bền vững hơn khi một kiểm soát riêng lẻ bị bỏ sót.
